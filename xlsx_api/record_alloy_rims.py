...
from xlsx_api.format_title import format_alloy_title
from xlsx_api.format_desc import format_rim_desc

def record_alloy_rims(cards: list, ws: worksheet, city_data: dict):
    cards_id_list = []
    title = 'Диски литые'
                ...
                if row[1].value not in cards_id_list:
                    ...
                    for card in cards:
                        if card['id'] == row[1].value:
                            if row[2].value == None:
                                row[2].value = format_alloy_title(card)
                            if row[3].value == None:
        ...
        if len(cards) > 0:
            ...
            for card in cards:
                ...
                if ws[index][1].value == None:
                    ...
                    ws[index][2].value = format_alloy_title(card)
